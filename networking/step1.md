...
**The prefix length is part of the address.** `10.9.9.10` without `/24` is not
an error, it silently means `/32`, and then nothing on the local network is
reachable.

Resolution is a separate mechanism again. `/etc/hosts` is a file, but nothing
reads it directly. The Name Service Switch decides that `hosts:` lookups
consult `files` before DNS, and that ordering in `/etc/nsswitch.conf` is what
makes a `hosts` entry win over a real DNS record. `getent hosts <name>` asks
through the switch, which is why it is the honest test, and `ping` is not.
