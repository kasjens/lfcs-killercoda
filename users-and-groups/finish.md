...
- **Section numbers describe the audience, not the importance.** `chage` is
  section 1 because you can run `chage -l` on yourself. `useradd` is 8 because
  you cannot usefully run it on yourself at all.
...
- **PAM modules are 8, their config files are 5**, and the config page is
  ...
  file.
- **`man -k` on the concept**, not the tool: "name service", "access control
  list", "password ageing".

Three traps that were in here on purpose:

1. `man profile` is not `/etc/profile`. Shell startup files are documented
   under INVOCATION in `bash(1)`.
