...
`mount(8)`{{}} is about nine screens. Scrolling it is how people lose four
minutes.
...
`mount(8)`{{}} or `nfs(5)`{{}} it turns two minutes of scrolling into one line
of typing.
...
Read 7 and 8 out of the page. Both are in the same list, so `&`{{}} earns its
keep here:
...
Inside the page, type `&exec`{{}} and press Enter. Every line mentioning exec,
and nothing else. Press `&`{{}} then Enter with an empty pattern to switch it
off. For question 8, `&suid`{{}} narrows it the same way.

Question 9 is in the pager's own help, which is `h`{{}} from inside any page,
or `man less`{{}} from outside it.
...
Both options are in the filesystem-independent list in `mount(8)`{{}}. The key
`G`{{}} goes to the end and `g`{{}} to the top, which is why the question said
case matters.
