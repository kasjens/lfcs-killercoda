...
- **`man -k word`** when you have forgotten the command name. If it says nothing
  appropriate, run `mandb`.
- **`man -K word`** when you have forgotten which page owns a directive. Slow, so
  narrow it: `man -K -s 5 word`.
- **`&word`** inside the pager, to collapse a nine-screen page to the six lines
  that matter.
- **`/EXAMPLES`** first, on any unfamiliar admin command. Most section 8 pages
  have working examples you can adapt faster than you can read the flag list.
