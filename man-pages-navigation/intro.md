...
In the LFCS exam you get man pages, `/usr/share/doc`{{}}, and the packages
installed on the box. Nothing else. No browser, no notes, no search engine.
...
That writes `5`{{}} to `/tmp/answers/1`{{}}, which is the file the check reads.
Nothing more to it, so redirection does the same job if you prefer:
...
> minute before you start, and `man -k quota`{{}} will tell you when it is
> ready.
