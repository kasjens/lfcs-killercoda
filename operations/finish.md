...
- **`systemd-analyze verify`** finds faults in a unit file that `daemon-reload`
  accepts without complaint.
- **`mount -a` and `findmnt --verify` before rebooting**, every time you touch
  `fstab`. Rebooting to find out is how an edit becomes a rescue-media
  afternoon.
- **`apt-cache policy <pkg>`** explains why apt chose what it chose, which no
  amount of reading the sources file will.
...
4. The `OnCalendar=` grammar is in `systemd.time(7)`, not `systemd.timer(5)`.
