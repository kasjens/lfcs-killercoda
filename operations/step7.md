...
suspects in `/var/log` look innocent. `max-size` and `max-file` in `log-opts`
bound it. This is the single most common cause of a full disk on a container
host, and it is one setting.
...
`docker info` prints the effective configuration, which is how you confirm a
setting took rather than assuming.
